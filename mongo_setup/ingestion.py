...
def clean_text(text):
    ...
    """
    # Remove any character that is not printable (ASCII 32-126)
    cleaned = re.sub(r'[^\x20-\x7E]+', ' ', text)
    return cleaned.strip()

def connect_to_mongo():
    """Connect to MongoDB and return the test_files collection."""
    try:
        client = MongoClient(MONGO_URI)
        db = client["Financial_Rag_DB"]
        if "test_files" in db.list_collection_names():
            collection = db["test_files"]
            print("✅ Using existing collection Financial_Rag_DB.test_files")
        else:
            collection = db["test_files"]
            print("✅ Created new collection Financial_Rag_DB.test_files")
        return collection
